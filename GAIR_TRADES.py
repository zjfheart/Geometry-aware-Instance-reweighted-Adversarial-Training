...
parser = argparse.ArgumentParser(description='GAIR-TRADES')
...
parser.add_argument('--out-dir',type=str,default='./GAIR_TRADES_result',help='dir of output')
...
parser.add_argument('--beta_trades',type=float,default=6.0,help='regularization parameter')
parser.add_argument('--nat_boost', type=bool, default=True, help='whether to adjust nat loss')
parser.add_argument('--adv_boost', type=bool, default=False, help='whether to adjust adv loss')
parser.add_argument('--begin_epoch', type=int, default=75, help='when to use GAIRLoss')
...
if args.lr_schedule == 'superconverge':
    lr_schedule = lambda t: np.interp([t], [0, args.epochs * 2 // 5, args.epochs], [0, args.lr_max, 0])[0]
        ...
        def lr_schedule(t):
            if t / args.epochs < 0.75:
                return args.lr_max
            elif t / args.epochs < 0.90:
...
if resume:
    ...
    print ('==> GAIR-TRADES Resuming from checkpoint ..')
    print(resume)
...
def train(epoch, model, train_loader, optimizer, beta):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = 0
        x_adv, pgd_steps = attack.pgd(model,data,target,args.epsilon,args.step_size,args.num_steps,loss_fn="kl",category="trades",rand_init=True)
        ...
        loss = TRADES_loss(logit, nat_logit, target, args.beta_trades, beta, pgd_steps)

        train_robust_loss += loss.item() * len(target)
